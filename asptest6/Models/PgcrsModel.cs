...
namespace asptest6.Models
{
    public class PgcrsModel
    {
        ...
        {
            Pgcr pgcr = null;
            string sql = $"SELECT json_object('pgcr_id', pgcr_id, 'pgcr', pgcr, 'flawless', flawless, 'starting_phase_index', starting_phase_index, 'raid_id', raid_id, 'player_count', player_count, 'time', time) from pgcrs WHERE pgcr_id = @pgcr_id";
        ...
        {
            string sql = "INSERT INTO pgcrs (pgcr_id, pgcr, flawless, starting_phase_index, raid_id, player_count, time) VALUES (@pgcr_id, @pgcr, @flawless, @starting_phase_index, @raid_id, @player_count, @time)";
            MySqlCommand cmd = new(sql, Database.Db);
            cmd.Parameters.AddWithValue("@pgcr_id", pgcr.pgcrId);
            ...
            cmd.Parameters.AddWithValue("@time", pgcr.TimeInSeconds);
            try
            {
                Database.Db.Open();
                MySqlDataReader reader = cmd.ExecuteReader();
